 ...
 * 全排列 -- 无重复元素
 */
public class AllCombinationWithNoDuplicate {

    public static void main(String[] args) {
        int[] arr = getRandomArraySizeOf(12);
        AllCombinationWithNoDuplicate allCombinationWithNoDuplicate = new AllCombinationWithNoDuplicate();
        long begin1 = System.currentTimeMillis();
        System.out.println("全排列无重复元素，回溯法实现结果：" +
                allCombinationWithNoDuplicate.getNumOfAllCombinationWithoutDuplicate(arr));
        long backTraceCost = System.currentTimeMillis();
        System.out.println("全排列无重复回溯元素耗时：" + (backTraceCost - begin1));

        long begin2 = System.currentTimeMillis();
        System.out.println("全排列无重复元素，邻里交换法实现结果：" +
                allCombinationWithNoDuplicate.getNumOfAllCombinationWithoutDuplicateWithSwapWay(arr));
        long swapCost = System.currentTimeMillis();
        System.out.println("全排列无重复交换元素耗时：" + (swapCost - begin2));
    }
    ...
    public int getNumOfAllCombinationWithoutDuplicate(int[] num) {
        //保证字典序
        Arrays.sort(num);
        this.nums = num;
     ...
     * @return
     */
    public int getNumOfAllCombinationWithoutDuplicateWithSwapWay(int[] num) {
        //保证字典序
        Arrays.sort(num);
        this.nums = num;
